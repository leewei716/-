...
@interface LXOtherPostController ()<ChatKeyBoardDelegate, ChatKeyBoardDataSource,UITableViewDelegate, UITableViewDataSource, ReloadMessageCellHightDelegate,UIScrollViewDelegate>


@property(nonatomic,retain)    UITableView *Otherposttable;
...
@implementation LXOtherPostController
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(OtherkeyboardWillShow:)
                                                     name:UIKeyboardWillShowNotification object:nil];
        //注册键盘隐藏NSNotification
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(OtherkeyboardWillHide:)
...
- (void)viewDidLoad {
    ...
    _Otherposttable=[[UITableView alloc]initWithFrame:CGRectMake(FLEXIBLE_NUMX(0), FLEXIBLE_NUMY(0), FLEXIBLE_NUMX(320), FLEXIBLE_NUMY(480))];
    //  隐藏分割线
    _Otherposttable.separatorStyle = UITableViewCellSeparatorStyleNone;
    _Otherposttable.delegate=self;
    _Otherposttable.dataSource=self;
    _Otherposttable.scrollEnabled=NO;
    [self.view addSubview:_Otherposttable];
    [self dealData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __weak __typeof(_Otherposttable) weakTable= _Otherposttable;
...
- (void)reloadCellHeightForModel:(MessageModel *)model atIndexPath:(NSIndexPath *)indexPath {
    ...
    [_Otherposttable reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
}

#pragma mark
#pragma mark keyboardWillShow
- (void)OtherkeyboardWillShow:(NSNotification *)notification
{
    ...
    CGPoint offset = _Otherposttable.contentOffset;
    offset.y += delta;
    if (offset.y < 0) {
        offset.y = -64;
    }
    [_Otherposttable setContentOffset:offset animated:YES];
}

#pragma mark
#pragma mark keyboardWillHide
- (void)OtherkeyboardWillHide:(NSNotification *)notification {
