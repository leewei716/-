...
{
    self.listArr = arr;
    _curText.font=[UIFont systemFontOfSize:14];
    _curText.text = self.listArr[0];
}

- (void)p_setUpView
{
    self.arrowImageName = @"more";
    
    _isOpen = NO;
    self.menuBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _menuBtn.frame = CGRectMake(0, 0, FLEXIBLE_NUMX(238), FLEXIBLE_NUMY(22));
    ...
    [_menuBtn addTarget:self action:@selector(tapActions) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_menuBtn];
    
    self.curText = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, FLEXIBLE_NUMX(228), self.frame.size.height)];
    ...
    _arrowImg.center = CGPointMake(FLEXIBLE_NUMX(230), self.frame.size.height/2);
    [_menuBtn addSubview:_arrowImg];
    
    self.menuTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, FLEXIBLE_NUMX(238), 0) style:UITableViewStylePlain];
...
//            NSDictionary * dic = @{@"hight":@(_menuTableView.frame.size.height)};
//            [[NSNotificationCenter defaultCenter]postNotificationName:@"closeHight" object:nil userInfo:dic];
//
        }];
    }else
    {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _arrowImg.transform = CGAffineTransformRotate(_arrowImg.transform, DEGREES_TO_RADIANS(180));
//            NSDictionary * dic = @{@"hight":@(_menuTableView.frame.size.height)};
//            [[NSNotificationCenter defaultCenter]postNotificationName:@"openHight" object:nil userInfo:dic];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGRect bounds = [string boundingRectWithSize:CGSizeMake(self.frame.size.width - 20, 200) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12] } context:NULL];
